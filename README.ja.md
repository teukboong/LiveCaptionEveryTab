...
- 翻訳：`Gemma-4 (full=26B-A4B / mid=E4B / lite=E2B)`（mlx-lm） — 既定は **quality プロンプト**（expert interpreter・by-meaning・no-translationese ＋ few-shot 3、KV-cache でコスト償却 → 硬い文語より自然な口語）。低遅延は `LCC_TX_PROFILE=fast`。**対象言語を選択可能**（韓/英/日/中/西/仏/独）、ソース自動判定、対象=ソースならスキップ。
