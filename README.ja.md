...
|---|---|---|---|---|
| **MLX** (`LCC_BACKEND=mlx`) | Apple Silicon | Granite/Qwen3（mlx-audio、インプロセス） | Gemma-4 · full/mid/lite（mlx-lm） | [SETUP.md](SETUP.md) |
| **CUDA** (`LCC_BACKEND=cuda`) | Windows + NVIDIA（WSL2） | Granite/Qwen3（transformers, `cuda/asr_server.py`） | llama.cpp · GGUF · full/mid/lite（OpenAI 互換 HTTP） | [SETUP-windows.md](SETUP-windows.md) |

文字起こしエンジンの選択（英語=granite／多言語=qwen3）は両者で同一（`model` フィールドでルーティング）— whisper は使いません。VAD・文ごとの組み立て・スケジューラ・ナンバーガード・プロンプトビルダーは**両 backend 共有**（純粋関数）で、ランタイムで変わるのは GPU の3関数（文字起こし／翻訳／要約）だけ。その境界が `bridge/backend_cuda.py`（HTTP）と server.py の "Backend seam" です。（コード既定は `mlx`。）
...
```
- ASR はポップアップで**2つの mlx-audio エンジン**から選びます（▸ 文字起こしエンジン）。**Granite Speech 4.1 2B**（`ibm-granite/granite-speech-4.1-2b`・英語に忠実、WER 0%台）と **Qwen3-ASR 1.7B**（`Qwen/Qwen3-ASR-1.7B`・日本語・韓国語を含む52言語、言語自動判定）。どちらも句読点・truecasing をネイティブに付与するので文のチャンク化がそのまま通ります。翻訳モデルと同じ Apple GPU を共有（直列）。⚠ granite は mlx-audio **main の conv 修正**が必要（SETUP 参照）。
...
- 翻訳：`Gemma-4 (full=26B-A4B / mid=E4B / lite=E2B)`（mlx-lm） — 既定は **quality プロンプト**（expert interpreter・by-meaning・no-translationese ＋ few-shot 3、KV-cache でコスト償却 → 硬い文語より自然な口語）。低遅延は `LCC_TX_PROFILE=fast`。**対象言語を選択可能**（45言語 — Gemma は多言語）、ソース自動判定、対象=ソースならスキップ。
- RAM ~26GB（full ティアの重み。mid ~8 / lite ~6GB はより小さい）＋チャンクごとに少量の KV。遅延 ~2.9–3.4s／発話チャンク（ASR ~0.7s ＋ 翻訳 ~1.4s ＋ 音声 prefill ＋ 節境界の待ち）。
...
あとは **拡張のポップアップが全部** やります — ブリッジ起動、そして **選んだティアだけ**（Full/Mid/Lite）モデルを取得してディスクを節約。（ターミナル派：`./setup.sh [--models --tier lite]`）
...
```
- 常時起動（オプトイン、クラッシュ時自動再起動）：`bash bridge/autostart.sh install` — ⚠ ~26GB RAM 常駐（full ティア）。停止：`… uninstall`
- ターミナルなしでポップアップのボタン（**ブリッジ起動**・モデル **Full/Mid/Lite**）で全部できます — ネイティブメッセージングホストが要りますが、**`./setup.sh` が既にインストール**します（ブラウザのサンドボックスが直接できない唯一のブートストラップ）。その後、拡張を再読み込み。detached なのでブラウザを閉じても維持（SETUP 6.5）。
