...
const LCC_CONTENT_FILES = ["protocol.js", "pcm.js", "page-seed.js", "caption-overlay.js", "caption-scheduler.js", "page-translator.js", "content.js", "delay.js"];
...
async function forward(msg) {
  ...
  else if ((msg.type === "dom_translate_result" || msg.type === "dom_translate_partial" || msg.type === "dom_translate_done" || msg.type === "dom_translate_busy" || msg.type === "dom_translate_err" || msg.type === "input_translate_result" || msg.type === "input_translate_err" || msg.type === "ocr_translate_result" || msg.type === "ocr_translate_err") && pageTabId != null) {
...
const LCC_TAB_SENDER_TYPES = new Set(["vd-pcm", "content-ready", "page-translate-batch", "input-translate", "ocr-capture"]);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "ocr-capture") {
    const tabId = sender && sender.tab && sender.tab.id;
    chrome.storage.session.get(["pageTranslating", "pageTabId"])
      .then(async ({ pageTranslating, pageTabId }) => {
        if (!pageTranslating || tabId == null || pageTabId !== tabId) return { ok: false, error: "페이지 번역이 켜진 탭에서만 됩니다" };
        const dataUrl = await chrome.tabs.captureVisibleTab({ format: "png" });   // rendered pixels — needs the activeTab grant from start
        await ensureOffscreen();
        const res = await chrome.runtime.sendMessage({ target: "offscreen", cmd: "ocr-crop", requestId: msg.requestId, dataUrl, rect: msg.rect, dpr: msg.dpr });
