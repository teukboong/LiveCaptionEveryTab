...
같은 브리지·같은 확장이 두 백엔드에서 그대로 돈다. 환경에 맞는 쪽을 `LCC_BACKEND`로 고른다.
...
3. 유튜브/트위치 영상 탭에서 **확장 아이콘 클릭** → 팝업에서 **페이지 번역 / 동영상 번역** 토글 선택 → **`자막 시작`** 클릭 → 배지 `ON`
4. 팝업 설정: 페이지 번역은 실제 DOM 텍스트를 순차 교체, 동영상 번역은 오버레이 자막 표시. 자막 **크기·상하/좌우 위치·원문 줄·싱크 보정** (실시간), **문장 대기·음성감지**(재시작 시 적용)
5. 다시 `자막 중지`. (tabCapture는 사용자 클릭 제스처 필수 → 자동시작 불가)

## 기능
- **용어 메모리 (자동 용어집)**: 자막에 반복 등장하는 이름·용어를 자동 채굴해 용어집에 핀 — 번역 렌더링 일관성 + 전사 철자 교정(fuzzy repair). 사이트(도메인)별로 기억해 재방문 시 자막·페이지 번역 양쪽에 자동 시드 (팝업에서 끄기)
- **듀얼 모델 동시성**: RAM이 넉넉하면 26B 옆에 작은 E2B 번역기를 자동 상주시켜 미리보기·페이지 DOM 번역을 분담 — 확정 자막은 항상 26B 품질, 페이지 번역은 자막과 경쟁하지 않음 (`LCC_AUX_LM=off`)
- **화자 구분 (베타)**: 팟캐스트·인터뷰에서 자막에 ①② 화자 라벨 — CPU 화자 임베딩 + 온라인 클러스터링, 첫 사용 시 ~109MB 모델 자동 다운로드
- **입력창 번역 (write-back)**: 외국 사이트 입력창에 내 언어로 쓰고 ⇄ 버튼/Alt+T로 페이지 언어로 변환(되돌리기 지원) — 읽기 렌즈를 '참여'로
- **이미지 번역 (macOS)**: Alt+이미지에 마우스 → Apple Vision OCR(ANE, 모델 다운로드 없음) → 짤·스크린샷 속 텍스트도 로컬 번역 오버레이
- **원문 같이 보기 (inline ghost)**: 번역된 긴 문단 아래에 원문을 옅게 상시 표시 — hover 없이도 원문 보존
- **iframe 포함 페이지 번역**: 임베드 위젯·iframe 본문까지 번역 (실콘텐츠 프레임 자동 감지)
