...
- page DOM 번역, 요약/질문, warm-up은 모두 `wait_aux_translation_slot()`을 통과해야 한다.
- caption final/preview backlog, active speech, ASR work queue가 있으면 DOM 번역은 `dom_translate_busy`로 양보하고 content script가 재시도한다.
