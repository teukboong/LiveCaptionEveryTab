...
- **aux 번역기 상주 시(MLX 듀얼 모델)**: 짧은 microbatch·per-item 폴백은 aux의 자체 풀/락에서 돌아
  ...
  `dom_translate_result`는 `engine:"aux"|"main"`을 싣고, pageVerify가 켜져 있으면 content는 aux가 칠한
  결과를 idle에 main으로 재검증 enqueue한다(speed layer 페인트 → quality layer 확인). verify 요청은
  배치 메시지에 `verify:true`를 실어 보내며 항상 main을 탄다.
