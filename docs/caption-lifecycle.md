...
자막 표시는 세 가지 `type`으로 나간다. 모두 `unit_id`/`rev`/`start_ms`/`end_ms`를 포함하고,
화자 구분(diarize)이 켜져 있으면 셋 다 `speaker`(1-base int 또는 null)를 추가로 싣는다 — client는
**두 번째 화자가 관측된 뒤부터** KO 라인에 ①② 접두를 붙인다(단일 화자 콘텐츠는 그대로).
자막 외 보조 채널: `term_memory`(자동 용어집 스트림 — SW가 도메인별 영속화),
`input_translate_result/_err`(입력창 write-back), `ocr_translate_result/_err`(이미지 OCR 오버레이).

### 2.1 `source` — 원문 라인 (번역 전)
```
{ type:"source", text, unit_id, rev, start_ms, end_ms, speaker }
```
...
|---|---|---|---|
| `aggressive` | latest-only 미리 번역 | 켬(게이트 통과 시) | GPU ASR(granite/qwen3/whisper) ∥ 번역 최대 겹침(분리된 device lock; parakeet은 CPU 탈출구) |
...
페이지 번역은 자막 오버레이가 아니다. top frame의 실제 text node를 직접 교체한다.

흐름:

1. popup의 `페이지 번역` 토글이 `runMode=page|both`를 만든다.
2. background가 `page-translate-start`를 content script에 보내고, offscreen에는 `start-page`를 보낸다.
3. content script는 `MutationObserver`와 scroll/resize scan으로 visible text node delta를 큐에 넣는다.
4. offscreen은 `dom_translate_batch`를 같은 bridge WebSocket으로 보낸다.
5. bridge는 `translate_once()`로 item별 번역을 만들고 `dom_translate_result`를 반환한다.
6. content script가 원래 text node가 아직 같은 값일 때만 `node.nodeValue`를 번역문으로 교체한다.

스케줄링:

- **aux 번역기 상주 시(MLX 듀얼 모델)**: 짧은 microbatch·per-item 폴백은 aux의 자체 풀/락에서 돌아
  자막과 경쟁하지 않는다(`dom_translate_busy` 양보 없음). 긴 문단·verify 재확인·ask는 main 모델 유지.
  `dom_translate_result`는 `engine:"aux"|"main"`을 싣고, content는 **aux 결과를 한 번이라도 보면**
  verify 파이프라인을 세션 동안 자동 활성화해 aux가 칠한 결과를 idle에 main으로 재검증·패치한다
  (speed layer 페인트 → quality layer 교정 — 듀얼 모델의 기본 동작; pageVerify 토글은 단일 모델에서
  캐시 라벨 재확인을 옵트인할 때만 필요). verify 요청은 배치 메시지에 `verify:true`를 실어 보내며
  항상 main을 탄다.
- aux가 없으면(단일 모델/CUDA): page DOM 번역, 요약/질문, warm-up은 모두 `wait_aux_translation_slot()`을
  통과해야 한다. caption final/preview backlog, active speech, ASR work queue가 있으면 DOM 번역은
  `dom_translate_busy`로 양보하고 content script가 재시도한다.
- CUDA backend에서도 같은 bridge-level `translate_once()` seam을 쓰므로 MLX/CUDA 모두 같은 우선순위 규칙을 따른다.
- **iframe**: 페이지 번역은 모든 실콘텐츠 프레임에서 돈다(프레임별 큐/캐시, 프레임 태그로 request id가
  탭-유일). 자막 오버레이 렌더 규칙(`lccShouldRender`)은 그대로다.

delta 규칙:

- 이미 번역된 node 뒤에 페이지가 새 suffix를 붙이면, content script는 저장된 원문 full text에 suffix를 합쳐 다시 번역한다.
- `script/style/input/pre/code/contenteditable/aria-hidden/#lcc-overlay` 등은 제외한다.
- stop 시 현재 session에서 직접 교체한 text node는 가능한 한 원문으로 복구한다.
...
- 페이지 DOM 번역 메시지/스케줄링/복구 규칙 변경(§8)
